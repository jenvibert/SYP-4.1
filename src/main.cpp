...
float avgSensorOutput (int sampleCount){
  ...
  delay(5);
  cap = getReadingFromFDCwithAddressAndBus(Wire, deviceArray[i][0], deviceArray[i][2], i);
  capValues[0][2]=cap;
  ...
  delay(5);
  cap = getReadingFromFDCwithAddressAndBus(Wire1, deviceArray[i][0], deviceArray[i][2], i);
  capValues[2][2]=cap;
